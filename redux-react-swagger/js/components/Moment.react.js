...
var Moment = React.createClass({
    ...
    render: function() {
        ...
        if(!this.state.editMode) {
            // Normal mode
            momentBody = (
                <p><span>{this.props.moment.text} </span></p>
            );
        } else {
            // Edit mode
            momentBody = (
                <p><textarea autoFocus ref="editMomentTextInput" onKeyDown={this._onSaveKey} defaultValue={this.props.moment.text} className="form-control" rows="3" /></p>
            );
        }
        ...
        return (
            <div className="panel panel-primary">
                <div className="panel-heading">
                    <strong>{this.props.moment.authorName}</strong>
                ...
                <CommentSection show={this.state.showComments} comments={this.props.moment.comments} parent={this.props.moment._id} />
            </div>
        );
    },
    ...
    _onSave: function() {
        AppActions.updateMoment(
            ...
            {
                text: this.refs.editMomentTextInput.value
            }
        );
    ...
    _toggleCommentsView: function() {
        ...
        if(!this.state.showComments) {
            AppActions.fetchCommentsFor(this.props.moment["_id"]);
        }
